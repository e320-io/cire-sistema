...
function POS({session,onSwitchSucursal,isAdmin,tema="dark",toggleTema=()=>{}}){
  ...
  const msiD=[...new Set(carrito.flatMap(i=>i.msi||[]))].sort((a,b)=>a-b);
            ...
            {filtro==="Mantenimiento"?(
              ...
              <div style={{padding:"14px 16px",background:"rgba(139,92,246,0.06)",border:"1px solid rgba(139,92,246,0.4)",borderRadius:"12px"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                  ...
                  <div style={{display:"flex",gap:"8px"}}>
                    <div style={{flex:"0 0 80px"}}><div style={{fontSize:"9px",color:T.sub,marginBottom:"4px",letterSpacing:"1px"}}>SESIONES</div><input type="number" className="inp" placeholder="Nº" value={zonasSesiones} onChange={e=>setZonasSesiones(e.target.value)} min="1" style={{fontSize:"12px",padding:"8px 10px"}}/></div>
                    <div style={{flex:"0 0 80px"}}><div style={{fontSize:"9px",color:T.sub,marginBottom:"4px",letterSpacing:"1px"}}>DUR. (min)</div><input type="number" className="inp" placeholder="60" value={zonasDuracion} onChange={e=>setZonasDuracion(e.target.value)} min="15" step="15" style={{fontSize:"12px",padding:"8px 10px"}}/></div>
                    <div style={{flex:"0 0 70px"}}><div style={{fontSize:"9px",color:T.sub,marginBottom:"4px",letterSpacing:"1px"}}>MSI</div><input type="number" className="inp" placeholder="—" value={zonasMsi} onChange={e=>setZonasMsi(e.target.value)} min="1" style={{fontSize:"12px",padding:"8px 10px"}}/></div>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"12px 18px",display:"flex",flexDirection:"column",gap:"14px"}}>
            ...
            <div><div style={{fontSize:"9px",letterSpacing:"1px",color:pOk?"#10b981":T.faint,marginBottom:"6px",display:"flex",alignItems:"center",gap:"5px"}}><div style={{width:"16px",height:"16px",borderRadius:"50%",background:pOk?"#10b981":T.dotBg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"8px",fontWeight:700,color:pOk?"#fff":T.faint,flexShrink:0}}>1</div>PAQUETE</div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
                ...
                {carrito.length>1&&<div style={{marginTop:"2px"}}>
                  {!showPromoManual&&!promoManualOk&&<button className="btn-ghost" style={{width:"100%",fontSize:"11px",padding:"7px",borderColor:"#f59e0b",color:"#f59e0b"}} onClick={()=>setShowPromoManual(true)}>🏷 Agregar precio promoción</button>}
                  {(showPromoManual||promoManualOk)&&<div style={{display:"flex",flexDirection:"column",gap:"6px",padding:"10px",background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.3)",borderRadius:"8px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:"9px",letterSpacing:"1px",color:"#f59e0b",fontWeight:600}}>PRECIO PROMOCIÓN</span>
                      <button onClick={()=>{setShowPromoManual(false);setPrecioPromoManual("");}} style={{background:"none",border:"none",color:"#f59e0b",cursor:"pointer",fontSize:"14px",lineHeight:1}}>×</button>
                    </div>
                    <input type="number" className="inp" placeholder="Monto especial $" value={precioPromoManual} onChange={e=>setPrecioPromoManual(e.target.value)} min="0" style={{fontSize:"13px",padding:"8px 10px",fontWeight:600}}/>
                    {promoManualOk&&<div style={{fontSize:"11px",color:"#f59e0b"}}>✓ Se cobrará {fmt(Number(precioPromoManual))} en vez de {fmt(total)}</div>}
                  </div>}
                </div>}
              </div>}
            </div>
      ...
      {showConfirm&&<div className="overlay"><div className="glass" style={{width:460,padding:"28px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"12px",marginBottom:"18px"}}>
          ...
          <div style={{padding:"14px",background:"rgba(0,0,0,0.3)",borderRadius:"10px"}}>
            ...
            {promoManualOk?<div style={{display:"flex",justifyContent:"space-between",fontSize:"13px",color:"#f59e0b",marginBottom:"6px"}}><span>🏷 Precio promoción</span><span>-{fmt(total-totalCD)}</span></div>:descuento>0&&<div style={{display:"flex",justifyContent:"space-between",fontSize:"13px",color:"#ff8a65",marginBottom:"6px"}}><span>Desc. {descuento}%</span><span>-{fmt(total*descuento/100)}</span></div>}
