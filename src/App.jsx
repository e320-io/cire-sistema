...
const ZONAS_PACK=["Cuerpo Completo","Piernas Completas","Medias Piernas","Brazos","Medios Brazos","Axilas","Espalda Completa","Media Espalda","Glúteos","Zona Interglútea","Abdomen","Línea Abdomen","Pecho","Pezones","Rostro Completo","Medio Rostro","Bigote","Mentón","Patillas","Bikini Brazilian","French Bikini","Sexy Bikini","Bikini Básico"];
          ...
          <div style={{flex:1,overflowY:"auto",padding:"16px 20px"}}>
            {filtro==="Mantenimiento"?(
              ...
              <div style={{padding:"14px 16px",background:"rgba(139,92,246,0.06)",border:"1px solid rgba(139,92,246,0.4)",borderRadius:"12px"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                  <div><div style={{fontSize:"9px",color:T.sub,marginBottom:"6px",letterSpacing:"1px"}}>ZONAS ({zonasSeleccionadas.length} seleccionadas)</div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:"5px",maxHeight:"120px",overflowY:"auto",paddingRight:"4px"}}>{ZONAS_PACK.map(z=>{const sel=zonasSeleccionadas.includes(z);return(<button key={z} onClick={()=>toggleZona(z)} style={{padding:"4px 10px",borderRadius:"20px",border:`1px solid ${sel?"rgba(139,92,246,0.7)":T.chipBdr}`,fontSize:"10px",fontWeight:sel?600:400,cursor:"pointer",background:sel?"rgba(139,92,246,0.2)":"transparent",color:sel?"#c4b5fd":T.muted,transition:"all 0.1s"}}>{z}</button>);})}</div>
                  </div>
                  <div style={{display:"flex",gap:"8px"}}>
                  ...
                  {zonasSeleccionadas.length>0&&zonasSesiones&&zonasPrecio&&<div style={{padding:"7px 10px",background:"rgba(139,92,246,0.08)",borderRadius:"8px",fontSize:"11px",color:T.muted}}>«Pack: {zonasSeleccionadas.join(", ")} ({zonasSesiones} ses{zonasDuracion?` · ${zonasDuracion} min`:""})» — {fmt(Number(zonasPrecio))}</div>}
