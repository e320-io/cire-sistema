...
function FichaClienta({clientaId,session,onClose,isAdmin=false}){
  ...
  return(
    <div style={{padding:"24px 28px",overflowY:"auto",flex:1,color:light?"#1a1a2e":"#fff"}}>
      ...
      {/* MODAL SESIÓN EXTRA */}
      {modalSesExtra&&<div className="overlay"><div className="glass" style={{width:500,maxHeight:"90vh",overflow:"auto",padding:"28px",borderColor:"rgba(245,158,11,0.4)"}}>
        <div style={{textAlign:"center",marginBottom:"20px"}}><div style={{fontSize:"28px",marginBottom:"8px"}}>⭐</div><div style={{fontSize:"16px",fontWeight:700,marginBottom:"4px"}}>Sesión extra</div><div style={{fontSize:"13px",color:T.muted}}>{clienta.nombre}</div></div>
        ...
        <div style={{display:"flex",gap:"8px",marginTop:"8px"}}><button className="btn-ghost" style={{flex:1}} onClick={()=>setModalSesExtra(false)}>Cancelar</button><button style={{flex:2,padding:"12px",borderRadius:"10px",border:"1px solid rgba(245,158,11,0.5)",background:"rgba(245,158,11,0.15)",color:"#f59e0b",fontSize:"13px",fontWeight:700,cursor:"pointer",opacity:(!sesExtraMotivo.trim()||!fechaSesExtra||!horaSesExtra||savingSesExtra||new Date(fechaSesExtra+"T12:00:00").getDay()===0)?0.4:1}} disabled={!sesExtraMotivo.trim()||!fechaSesExtra||!horaSesExtra||savingSesExtra||new Date(fechaSesExtra+"T12:00:00").getDay()===0} onClick={agendarSesExtra}>{savingSesExtra?"Guardando...":"✓ Confirmar sesión extra"}</button></div>
      </div></div>}
